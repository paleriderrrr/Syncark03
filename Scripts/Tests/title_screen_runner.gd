...
func _run() -> void:
	...
	var fog_mask: TextureRect = screen.get_node_or_null("FogMask")
	_assert(fog_mask != null, "Fog mask should exist above the main backdrop")
	if fog_mask != null:
		_assert(fog_mask.get_index() > cover_base_1.get_index(), "Fog mask should render above CoverBase1 so the mist is visible")
		_assert(fog_mask.texture == cover_base_1.texture, "Fog mask should reuse CoverBase1 texture as its visible mask source")
